...
def encryption(user_input, original_key):
    ...
    for round_num in range(0, 16):
        ...
        if (round_num != 16):
            print("Round ", round_num + 1, " ", rpt,
                  " ", lpt, " ", round_keys[round_num])
    ...
    print("final cipher asci: ", final_cipher_ascii)
...
def decryption(final_cipher, original_key):
    # Initialize lists to store round keys
    round_keys = generate_round_keys(original_key)
...
key_input = input("Enter a key (8 characters): ")

while not validate_key_size(key_input):
...
for i in range(0, len(user_input), 8):
    partial_input = user_input[i:i+8]
    enc += encryption(partial_input, key_input)

# Decryption
dec = ''
for i in range(0, len(enc), 64):
    partial_enc = enc[i:i+64]
    enc_to_binary = str_to_bin(partial_enc)
