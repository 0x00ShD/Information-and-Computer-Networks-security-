...
def key_expansion(key: bytes, nb: int = 4) -> [[[int]]]:
    ...
    for i in range(nk, nb * (nr + 1)):
        ...
        ff= [w[i*4:(i+1)*4] for i in range(len(w) // 4)]
...
def aes_encryption(data: bytes, key: bytes) -> bytes:
    ...
    for round in range(1, nr):
        print("\nRound {} (Round Key):".format(round))
        print(key_schedule[round])
        sub_bytes(state)
        shift_rows(state)
        mix_columns(state)
        add_round_key(state, key_schedule, round)

    print("\nRound {} (Round Key):".format(nr))
    print(key_schedule[nr])
...
if __name__ == "__main__":
    ...
    text = "shadymohamedezze"

    plaintext_hex =text.encode("utf-8").hex()
    print("plaintext in hex " ,plaintext_hex)

    valid_pt_block = valid_block_size(plaintext_hex)
    plaintext_matrix = key_and_text_to_matrix(plaintext_hex)
    ...
    plaintext_hex1=bytearray.fromhex(plaintext_hex)
    key1 = bytearray.fromhex(key)

    
    ciphertext = aes_encryption(plaintext_hex1, key1)

    recovered_plaintext = aes_decryption(ciphertext,key1)


    hex_string = ciphertext.hex()
    print("ecrypted hex_string :  ",hex_string)
    print("------------------------------------------\n")
    

    print("dcrypted massege    :  ",recovered_plaintext)